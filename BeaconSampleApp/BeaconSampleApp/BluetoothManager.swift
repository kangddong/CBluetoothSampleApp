...
//        if let name = peripheral.name, name == "MBeacon" {
//            if let dic = advertisementData["kCBAdvDataServiceData"] as? Dictionary<CBUUID, Data>, let data = dic[CBUUID(string:"FFF1")], data.count == 11  {
...
//                let distance = rssiToDistance(rssi: RSSI)
//                print("DiscoverBle(name: \(name), macAddress: \(strAddress), rssi: \(RSSI), distance: \(distance)")
//                
//                delegate?.serialDidDiscoverBle(address: strAddress, RSSI: RSSI, distance: distance)
//            }
//        }
